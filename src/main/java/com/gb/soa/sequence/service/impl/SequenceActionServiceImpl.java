...
import com.gb.soa.sequence.util.DateUtil;
...
public class SequenceActionServiceImpl implements SequenceActionService  {
	...
	public CreateSequence getSequenceToClient(CreateSequence createSequence) {
		...
		try {
			...
			boolean belongCalendar = DateUtil.belongCalendar("common");
			if (belongCalendar) {
	...
	public String getAutomicSeq(String seqName, Integer num, Long tenantNumId, Long dataSign) {
		...
		boolean belongCalendar = DateUtil.belongCalendar("auto");
		if (belongCalendar) {
		...
		try {
			...
			if (!lock.tryLock()) {
				...
				if (redisCurrentVal == null || redisCurrentVal.trim().equals("")) {
					// 第一次进入
					if (currentNum == 1L || (initValue != null && initValue == currentNum)) {
				...
				if (isFlowCode != 6) {// 如果是6不需要流水
					...
					if (flowCodeLength > 0) {
						...
						if (currentLength <= flowCodeLength.intValue()) {
							String resultStringVal = SeqStringUtils.frontCompWithZore(currentSeqVal,
